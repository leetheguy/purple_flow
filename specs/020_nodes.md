...
- 2026-09-26 — [090](090_workflow_files.md) (draft): **Code**: edits to a `.exs` file take effect on their own within about two seconds, with no `reload/0`. A Code node's `file` must stay inside the workflows folder. `prepare(config, node_dir)` becomes `prepare(config, node_dir, root)`, where `root` is the workflows folder, so a node can resolve files it names without leaving it.
